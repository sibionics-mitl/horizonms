...
class LRFinder(object):
    ...
    def __init__(
        ...
        if "cuda" in str(self.model_device):
            self.device = "cuda"
        else:
            self.device = "cpu"
        # print("self.device: ", self.device, str(self.model_device))
    ...
    def range_test(
        ...
        Arguments:
            ...
            val_loader (`torch.utils.data.DataLoader`
                or child of `ValidDataLoaderIter`, optional): if `None` the range test
                ...
                you should inherit from `ValidDataLoaderIter` class and
        ...
        if val_loader:
            if isinstance(val_loader, DataLoader):
                val_iter = ValidDataLoaderIter(val_loader)
            elif isinstance(val_loader, ValidDataLoaderIter):
                val_iter = val_loader
            else:
                raise ValueError(
                    "`val_loader` has unsupported type: {}."
                    "Expected types are `torch.utils.data.DataLoader`"
                    "or child of `ValidDataLoaderIter`.".format(type(val_loader))
                )
    ...
    def _train_batch(self, train_iter, accumulation_steps, non_blocking_transfer=True):
        ...
        for i in range(accumulation_steps):
            ...
            # Forward pass
            loss_dict, outputs = self.model(inputs, labels)
    ...
    def _validate(self, val_iter, non_blocking_transfer=True):
        ...
        return running_loss / len(val_iter.data_loader)
    ...
    def plot(
        ...
        history.to_csv(os.path.join(self.cache_dir, 'lr_range_test.csv'))
        print(lrs)
        print(losses)
        if skip_end == 0:
            lrs = lrs[skip_start:]
        ...
        if fig is not None:
            plt.show()
            plt.savefig(os.path.join(self.cache_dir, 'lr_range_test.png'))
